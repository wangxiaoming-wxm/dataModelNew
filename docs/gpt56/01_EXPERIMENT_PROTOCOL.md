...
```json
{
  ...
  "parent": "v2",
  "git_commit": "<sha>",
  "train_sha256": "494a6107...",
  "test_sha256": "d6ffd26b...",
  "view": "main",
  "arms": ["cat_d5"],
  "folds": 10,
  "seeds": [20260],
...
`[-0.00079, +0.00476]`，`P(Δ>0)=86.4%`。因此旧文档担心 `max` 的抽样标准差
...
- 编码世界最多同时筛 12 个；
- 晋级：`ΔAUC ≥ +0.0015`；
- 若与父模型秩相关 ≤0.94，可放宽为 `ΔAUC ≥ +0.0008`。
...
- 通过标准：
  - 确认集方向为正；
  - discovery + confirmation 合并后 `P(Δ>0) ≥ 90%`；
  - 至少 6/8 个 seed 的差值为正；
  - 没有数据校验、格式或打乱标签异常。

大量候选中选最好者后直接报同一批 OOF，会产生选择乐观；两级制度是 S3 编码世界工厂的硬要求。
...
外层 5 块，在 4 块上选规则，在第 5 块上评分。除了最终 `nested_oof_auc`，还要记录：
...
若 5 块不能至少 4 块选中同一规则，最终使用前两条稳定规则的平均，而不是多数票险胜的单一规则。
...
```text
id,parent,hypothesis,commit,status,folds,seeds,oof,delta,ci90_low,ci90_high,
positive_seeds,corr,cost_sec,decision,notes
```
