...
NR=2; splits=make_splits(y,5,NR); CKPT=[400,600,800,1100]
der=_derive(tr,edges["__scale__"])
for tag,nv,noise in [("noise+J2",2,True),("noise+J4",4,True),("J4_only",4,False)]:
    X,cats=build(tr,edges,"cross2")
    if noise: add_noise_view(X,cats,tr)
    add_jitter_views(X,cats,tr,der["cond_r"],pd.to_numeric(tr["days"]),n_views=nv)
    ...
    for r,f,ti,vi in splits:
        m=CatBoostClassifier(loss_function="Logloss",iterations=max(CKPT),learning_rate=0.03,depth=6,
                             l2_leaf_reg=10,random_strength=0.7,verbose=False,thread_count=4,
    ...
    print(f"{tag} nfeat={X.shape[1]} ncat={len(cats)} {line} {time.time()-t0:.0f}s",flush=True)
