#!/usr/bin/env python3
"""第一性原理 v6：在 fp_v5 上加入 within-byte OR / triple-AND / XOR 池。

证据：bit-AND 二阶已近正交抬升；自然扩展为同构的选择无关池：
  - within-byte bit OR（每字节 C(8,2)）
  - within-byte bit triple-AND（每字节 C(8,3)）
  - within-byte bit XOR（每字节 C(8,2)）

预注册主交（heavy_xor；相对 and_tri_or 再抬一档，仍保留表格臂）：
    score = 0.20*v3 + 0.15*bits + 0.05*xs + 0.30*and_all
          + 0.10*tri + 0.10*or + 0.10*xor

门禁：nested fold-mean > fp_v5；锚点 OOF ≈ 0.743417。
备份：tempered=and_tri_or；aggressive=max_id_signal。
"""
...
# champion: heavy_xor
W_V3, W_BITS, W_X, W_AND = 0.20, 0.15, 0.05, 0.30
W_TRI, W_OR, W_XOR = 0.10, 0.10, 0.10
EXPECTED_OOF = 0.7434166163115536
OOF_ATOL = 1e-8
V5_OOF = 0.7288000993568079

# backups
TEMPERED = dict(v3=0.22, bits=0.18, xs=0.10, and_all=0.30, tri=0.10, or_=0.10, xor=0.0)
AGGRESSIVE = dict(v3=0.15, bits=0.15, xs=0.05, and_all=0.35, tri=0.10, or_=0.10, xor=0.10)
        ...
        for i in range(8):
            for j in range(i + 1, 8):
                for k in range(j + 1, 8):
                    tr_keys.append(
                        pd.Series(
                            (bits_tr[:, b * 8 + i] & bits_tr[:, b * 8 + j] & bits_tr[:, b * 8 + k]).astype(str)
                        )
                    )
                    te_keys.append(
                        pd.Series(
                            (bits_te[:, b * 8 + i] & bits_te[:, b * 8 + j] & bits_te[:, b * 8 + k]).astype(str)
                        )
                    )
    return pool_keys(tr_keys, te_keys, y)
...
def main() -> int:
    ...
    if cache_ok:
        ...
        print("computing bits / and / or / tri / xor / xs pools...", flush=True)
    ...
    fuse_o = fuse_weights(parts_o, champ_w)
    fuse_t = fuse_weights(parts_t, champ_w)
    ...
    if oof_auc <= V5_OOF + 1e-15:
        raise SystemExit(f"GATE FAIL vs v5: {oof_auc} <= {V5_OOF}")

    tempered_w = {
        "v3": TEMPERED["v3"],
        "bits": TEMPERED["bits"],
        "xs": TEMPERED["xs"],
        "and_all": TEMPERED["and_all"],
        "tri": TEMPERED["tri"],
        "or": TEMPERED["or_"],
        "xor": TEMPERED["xor"],
    }
    agg_w = {
        "v3": AGGRESSIVE["v3"],
        "bits": AGGRESSIVE["bits"],
        "xs": AGGRESSIVE["xs"],
        "and_all": AGGRESSIVE["and_all"],
        "tri": AGGRESSIVE["tri"],
        "or": AGGRESSIVE["or_"],
        "xor": AGGRESSIVE["xor"],
    }
    temp_o = fuse_weights(parts_o, tempered_w)
    temp_t = fuse_weights(parts_t, tempered_w)
    agg_o = fuse_weights(parts_o, agg_w)
    agg_t = fuse_weights(parts_t, agg_w)
    ...
    out_path = ROOT / "submissions" / "submission_fp_v6.csv"
    champ = ROOT / "submissions" / "submission_champion.csv"
    tempered_path = ROOT / "submissions" / "submission_fp_v6_tempered.csv"
    aggressive_path = ROOT / "submissions" / "submission_fp_v6_aggressive.csv"
    ...
    if not args.verify_only:
        ...
        # 不覆盖全局 champion（由 fp_v7 持有）；仅写 v6 备份链

        tsub = sample[["id"]].copy()
        tsub["label"] = np.clip(temp_t, 0.001, 0.999)
        ...
        np.save(art / "v6_fuse_oof.npy", fuse_o)
        np.save(art / "v6_fuse_test.npy", fuse_t)

    # v6 已不再是全局 champion；校验以 submission_fp_v6.csv 为准
    ...
    metrics = {
        ...
        "formula": (
            f"{W_V3}*v3 + {W_BITS}*bits + {W_X}*xs + {W_AND}*and_all "
            f"+ {W_TRI}*tri + {W_OR}*or + {W_XOR}*xor"
        ),
        "weights": champ_w,
        "oof_auc": oof_auc,
        "nested_fold_mean": nest,
        "delta_nest_vs_v5": nest - 0.728997978438028,
        "delta_full_vs_v5": oof_auc - V5_OOF,
        "arm_aucs": {
        ...
        "spearman_xor_vs_and": float(spearmanr(xor_o, and_o).statistic),
        "spearman_or_vs_and": float(spearmanr(or_o, and_o).statistic),
        "tempered": {
            "name": "and_tri_or",
            "weights": {**{k: tempered_w[k] for k in tempered_w}},
            "oof": float(roc_auc_score(y, temp_o)),
            "sha256": sha256(tempered_path) if tempered_path.is_file() else None,
        },
        "aggressive": {
            "name": "max_id_signal",
            "weights": {**{k: agg_w[k] for k in agg_w}},
            "oof": float(roc_auc_score(y, agg_o)),
            "sha256": sha256(aggressive_path) if aggressive_path.is_file() else None,
        },
        "fp_v5_oof": V5_OOF,
        "submission": "submissions/submission_fp_v6.csv",
        "submission_sha256": sha256(check_path),
        "EXPECTED_OOF": EXPECTED_OOF,
        "gate_beat_v5": True,
        "note": "global champion is fp_v7; v6 retained as tempered backup",
    }
    (art / "v6_metrics.json").write_text(json.dumps(metrics, ensure_ascii=False, indent=2) + "\n")
    print("PASS: fp_v6 heavy_xor 超过 fp_v5")
    print(f"OOF={oof_auc:.8f} nest={nest:.8f} (v5={V5_OOF:.8f}, Δ={oof_auc - V5_OOF:+.8f})")
