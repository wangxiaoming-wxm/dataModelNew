#!/usr/bin/env python3
"""Train the 715-style semantic FE + CatBoostRegressor(RMSE) arm.
...
Usage:
  ...
  python3 src4/train_semantic_rmse.py
  python3 src4/train_semantic_rmse.py --seed 2026   # single seed part
"""
...
def run_seed(seed: int, train: pd.DataFrame, test: pd.DataFrame, y: np.ndarray, threads: int):
    ...
    for fold, (tri, vali) in enumerate(skf.split(train, y)):
        ...
        fold_aucs.append(fa)
        print(
            f"[semantic_rmse] seed={seed} fold={fold} auc={fa:.5f} ({time.time()-t0:.0f}s)",
            flush=True,
        )
    test_pred /= N_SPLITS
...
def main():
    ...
    if args.smoke:
        ...
        ITER = 200
        ES = 40
        print("=== SMOKE semantic_rmse ===", flush=True)
