...
本交付用 `src_super/features_te.py` 做了不训练模型的独立外层 5-fold 探针：只把验证行的诚实 TE 值直接当分数，AUC 为 **0.600575**，取值范围 `[0.018559, 0.218261]`。这证明该格子统计本身含信号且数值正常，但明显不能单独充当 `>0.697` 的第三臂；完整 CatBoost 臂仍必须过门槛。另用 80 个唯一 key 的合成数据验证，所有内层训练 OOF 编码都严格回退为 0.5，确认训练行未读取自身标签。
