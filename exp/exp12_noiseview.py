...
from features import fit_edges, build, add_noise_view

tr,te=load_raw(); y=tr["label"].to_numpy()
allf=pd.concat([tr.drop(columns=["label"]),te],ignore_index=True)
edges=fit_edges(allf)
NR=2; splits=make_splits(y,5,NR); CKPT=[300,500,700,1000]
for tag,lvl,noise in [("cross2+noise","cross2",True),("cross+noise","cross",True)]:
    X,cats=build(tr,edges,lvl)
